...
int main() {
    ...
    int tcs;
    int slcs;
    cout<<"Chỉ sửa lại có nhập 1 ,không nhập 0 :";
    cin>>cs;
    if(cs==1){
    cout<<"so luong hoc sinh can chinh sua :";
    cin>>slcs;
    
    for (int i = 0; i < slcs; i++){
    cout<<"chinh sua hoc sinh :";
    cin>>tcs;
    
     float tdtx=0;
     float dtx[sldtx];
     cout<<"____Hoc Sinh "<<tcs<<"____"<<endl;
     for (int l = 0; l < sldtx; l++){
      cout<<"Diem thuong xuyen so "<<l+1<<": ";
      cin>>dtx[l];
      tdtx+=dtx[l];
     ...
     cin>>dhk[1];
     cout<<endl;
     dtb[tcs-1]=(tdtx+(dhk[0]*hsghk)+(dhk[1]*hschk))/(sldtx+hsghk+hschk);
